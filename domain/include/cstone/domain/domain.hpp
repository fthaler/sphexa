...
public:
    ...
    /*! @brief Like syncGrav(), but using caller-supplied SFC rank boundaries instead of a computed assignment.
     *
     * Performs the same operations as syncGrav(): SFC-sorts particle arrays, builds the focused octree,
     * discovers halos, reorders particle buffers into their final layout, and exchanges halo particles.
     * The only difference is that the global domain decomposition assignment is fixed by @p rankBoundaries
     * rather than being computed from scratch, so no inter-rank particle migration takes place.
     *
     * @param[in]    rankBoundaries  SFC key boundaries for all ranks, length numRanks + 1.
     *                               Must satisfy rankBoundaries[numRanks] == nodeRange<KeyType>(0)
     *                               and exactly one entry must equal 0 (the rank owning the SFC origin).
     *                               Entry @p rank gives the start of rank @p rank's assigned SFC range.
     *                               The entries do NOT need to be in sorted order; makeFixedSfcAssignment
     *                               sorts them internally to build the spanning tree.
     * @param[out]   particleKeys    SFC keys for the local particles, sorted on output.
     * @param[inout] x,y,z           Particle coordinates; reordered into SFC order and extended with halos on output.
     * @param[inout] h               Interaction radii; reordered and extended with halos on output.
     * @param[inout] m               Masses / charges; reordered and extended with halos on output.
     * @param[inout] particleProperties  Additional particle properties; reordered and extended with halos on output.
     * @param[-]     scratchBuffers  Temporary storage (same constraints as syncGrav).
     *
     * After returning:
     *   - All particle arrays are SFC-sorted and include halo particles (same layout as after syncGrav()).
     *   - focusTree() contains a valid focused LET built around the provided assignment.
     *   - startIndex() / endIndex() give the range of locally-assigned particles within the extended arrays.
     */
    template<class KeyVec, class VectorX, class VectorH, class VectorM, class... Vectors1, class... Vectors2>
    void syncGravWithFixedAssignment(std::span<const KeyType> rankBoundaries,
                                     KeyVec& particleKeys,
                                     VectorX& x,
                                     VectorX& y,
                                     VectorX& z,
                                     ...
                                     std::tuple<Vectors2&...> scratchBuffers)
    {
        ...
        LocalIndex nLocal = x.size();
        bufDesc_          = {0, nLocal, nLocal};
        prevBufDesc_      = bufDesc_;

        // Compute SFC keys, build sort permutation in sorter buffer, and SFC-sort x,y,z,h,m.
        // This mirrors what distribute() does for the locally-owned range after particle exchange.
        reallocate(particleKeys, nLocal, allocGrowthRate_);
        computeSfcKeys<HaveGpu<Accelerator>{}>(rawPtr(x), rawPtr(y), rawPtr(z), sfcKindPointer(rawPtr(particleKeys)),
                                               nLocal, box());
        sequence<HaveGpu<Accelerator>{}>(0, nLocal, sfcOrder, allocGrowthRate_);
        sortByKey<HaveGpu<Accelerator>{}>(std::span{rawPtr(particleKeys), size_t(nLocal)},
                                          std::span{sorter.getMap(), size_t(nLocal)}, std::get<0>(scratch),
                                          std::get<1>(scratch), allocGrowthRate_);
        std::span<const KeyType> keyView(rawPtr(particleKeys), nLocal);
        // Reorder x,y,z,h,m into SFC order now (before halo discovery uses coordinates).
        gatherArrays({sorter.getMap(), size_t(nLocal)}, 0, std::tie(x, y, z, h, m), util::reverse(scratch));

        std::vector<KeyType> spanningLeaves;
        std::vector<LocalIndex> perRankCounts(numRanks_);
        MPI_Allgather(&nLocal, 1, MPI_INT, perRankCounts.data(), 1, MPI_INT, MPI_COMM_WORLD);
        SfcAssignment<KeyType> fixedAssignment =
            makeFixedSfcAssignment<KeyType>(rankBoundaries, perRankCounts, spanningLeaves);

        OctreeData<KeyType, CpuTag> fixedGlobalTree;
        fixedGlobalTree.resize(nNodes(spanningLeaves));
        updateInternalTree<KeyType>(spanningLeaves, fixedGlobalTree.data());
        std::span<const KeyType> globalLeavesSpan(spanningLeaves);

        auto fixedGlobalOctreeView   = fixedGlobalTree.cdata();
        fixedGlobalOctreeView.leaves = spanningLeaves.data();
        std::vector<unsigned> fixedNodeCounts(nNodes(spanningLeaves), 1u);

        if (firstCall_)
        {
            focusTree_.converge(box(), keyView, fixedAssignment, globalLeavesSpan,
                                std::span<const unsigned>(fixedNodeCounts), 1.0, std::get<0>(scratch));
            focusTree_.updateMinMac(fixedAssignment, 1.0, false);
            int converged = 0, reps = 0;
            while (converged != numRanks_ || reps < 2)
            {
                converged = focusTree_.updateTree(fixedAssignment, globalLeavesSpan, box(), std::get<0>(scratch));
                focusTree_.updateCounts(keyView, globalLeavesSpan, std::span<const unsigned>(fixedNodeCounts),
                                        std::get<0>(scratch));
                focusTree_.updateCenters(rawPtr(x), rawPtr(y), rawPtr(z), rawPtr(m), fixedGlobalOctreeView,
                                         std::get<0>(scratch), std::get<1>(scratch));
                focusTree_.updateMacs(fixedAssignment, 1.0 / theta_, false);
                MPI_Allreduce(MPI_IN_PLACE, &converged, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
                reps++;
            }
        }
        ...
        do
        {
            focusTree_.updateMacs(fixedAssignment, centerDriftTol_ / theta_, true);
            focusTree_.updateTree(fixedAssignment, globalLeavesSpan, box(), std::get<0>(scratch));
            focusTree_.updateCounts(keyView, globalLeavesSpan, std::span<const unsigned>(fixedNodeCounts),
                                    std::get<0>(scratch));
            focusTree_.updateCenters(rawPtr(x), rawPtr(y), rawPtr(z), rawPtr(m), fixedGlobalOctreeView,
                                     std::get<0>(scratch), std::get<1>(scratch));
        ...
        // Resize arrays to final layout (assigned + halos) and reorder unordered buffers.
        // For the fixed-assignment case there is no post-exchange particle migration, so the
        // sorter map covers [0, nLocal) starting at offset 0 — equivalent to updateLayout
        // with postExchangeStart == 0 and numAssigned == nLocal.
        {
            ...
            lowMemReallocate(newBufDesc.size, allocGrowthRate_,
                             std::tuple_cat(std::tie(x, y, z, h, m), particleProperties), scratch);
            layoutAcc_ = layout_;

            // re-locate particle SFC keys into the assigned window
            constexpr int i = util::FindIndex<KeyVec&, decltype(scratch), SmallerElementSize>{};
            constexpr int j = (i >= std::tuple_size_v<decltype(scratch)>) ? 0 : i;
            auto& swapSpace = std::get<j>(scratch);
            ...
            reallocate(particleKeys, newBufDesc.size, allocGrowthRate_);
            fill<HaveGpu<Accelerator>{}>(rawPtr(particleKeys) + bufDesc_.size, rawPtr(particleKeys) + newBufDesc.size,
                                         KeyType(0));
            copy_n<HaveGpu<Accelerator>{}>(swapPtr, keyView.size(), rawPtr(particleKeys) + newBufDesc.start);
            reallocate(swapSpace, origSize, 1.0);

            // relocate ordered buffers (x,y,z,h,m are already SFC-sorted at offset 0;
            // move them to newBufDesc.start inside the enlarged array)
            auto reversedScratch = util::reverse(scratch);
            auto relocate        = [size = keyView.size(), dest = newBufDesc.start, &reversedScratch](auto& array)
            {
                auto& sw = util::pickType<decltype(array)>(reversedScratch);
                copy_n<IsDeviceVector<std::decay_t<decltype(array)>>{}>(rawPtr(array), size, rawPtr(sw) + dest);
                swap(array, sw);
            };
            util::for_each_tuple(relocate, std::tie(x, y, z, h, m));

            // reorder unordered buffers (particleProperties) — sort map starts at index 0
            gatherArrays({sorter.getMap(), size_t(nLocal)}, newBufDesc.start, particleProperties, reversedScratch);
...
private:
    ...
    void diagnostics(size_t assignedSize)
    {
        ...
        auto fPeerFlags = focusPeers<KeyType>(global_.assignment().treeOffsetsConst(), focusAssignment, myRank_,
