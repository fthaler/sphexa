...
/*! @brief Specialized scatter for util::array<T,N>: each GPU thread scatters one scalar element.
 *  Launches numElements*N threads so that every T within each array is handled independently.
 */
template<class T, std::size_t N, class IndexType>
extern void scatterGpu(const IndexType* ordering, size_t numElements, const util::array<T, N>* src,
                       util::array<T, N>* buffer);
